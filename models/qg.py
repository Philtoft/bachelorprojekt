...
class QG:
    ...
    def evaluate(self):
        ...
        references: list[dict] = [entry['questions'].replace(" {sep_token}", "") for entry in squad['validation']]
        ...
        input_strings: list[str] = ["generate questions: " + entry['context'] + " </s>" for entry in squad['validation']]
        input_encodings = self._tokenizer(input_strings, padding='longest', truncation=True, return_tensors='pt').to(device)
        results = self._model.generate(**input_encodings, **generator_args).to(device)
        questions = self._tokenizer.decode(results, skip_special_tokens=True)
        print(questions)
